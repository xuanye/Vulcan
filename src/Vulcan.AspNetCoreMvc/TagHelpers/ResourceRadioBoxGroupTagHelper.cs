...
{
    [HtmlTargetElement("rs-radiobox")]
    public class ResourceRadioBoxGroupTagHelper: TagHelper
    {
        private const string ForAttributeName = "asp-for";


        private readonly IResourceService _service;    
        public ResourceRadioBoxGroupTagHelper( IResourceService service) 
        {
            _service = service;
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            {
                sb.AppendFormat(@"<div class='radio'><label><input type='radio' class='{4}' value='{0}' name='{1}' {2}/><span class='lbl'>{3}</span></label></div>",
                    resource.Value,
                    For.Name,
